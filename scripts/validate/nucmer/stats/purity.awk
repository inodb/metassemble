...
END {
    ...
    print "== ASSEMBLY =="
    print "name","purity","l50","n50","trim_n","max_contig_length","cut_off","trim_n_mapping","sum_ref_lengths","sum_max_qcov_bases","ratio","sum_bases"
    print ARGV[1],sum_max_qcov_bases / sum_bases,l50,n50,nr_contigs,all_contig_lengths[nr_contigs],cut_off,nr_contigs_mapping,sum_ref_lengths,sum_max_qcov_bases,sum_max_qcov_bases/ sum_ref_lengths,sum_bases

    print "== CONTIG LENGTH vs PURITY =="
    print "contig","contig_length","max_qcov"
    for (contig in max_qcov) {
        print contig,contig_length[contig],max_qcov[contig]
    }
}
